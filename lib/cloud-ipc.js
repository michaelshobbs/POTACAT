...
function registerCloudIpc(ctx) {
  ...
  ipcMain.handle('cloud-forgot-password', async (_e, email) => {
    try {
      const sync = getCloudSync();
      // Public endpoint; auth not required. _post(skipAuth=true)
      // bypasses the JWT injection so this works when the user is
      // signed out (the common case for "forgot password").
      await sync._post('/v1/auth/forgot-password', { email: String(email || '').trim() }, true);
      return { success: true };
    } catch (err) {
      return { error: err.message };
    }
