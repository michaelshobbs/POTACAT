...
class KenwoodCodec extends EventEmitter {
  ...
  // Monitor on/off uses ML0001;/ML0000; — single channel 0, level 000=off.
  setMonitor(on) {
    if (!this._isYaesu()) return;
    this._write(on ? 'ML0050;' : 'ML0000;');
  }

  setMonLevel(pct) {
    if (!this._isYaesu()) return;
    const v = Math.max(0, Math.min(100, Math.round(pct)));
    this._write(`ML0${String(v).padStart(3, '0')};`);
  }
  ...
  // FTX-1 clarifier — split RX/TX enable, shared CF offset.
  // RX enable maps to RIT (RT), TX enable maps to XIT (XT). The actual
  // offset is written via CF (signed Hz, 4-digit padded + sign byte) and
  ...
  setClarTx(on) {
    if (!this._isYaesu()) return;
    this._write(on ? 'XT1;' : 'XT0;');
  }

  // CF<sign><4-digit>; — sign is '+' or '-', value in Hz, clamped 0..9999.
  setClarOffset(hz) {
    if (!this._isYaesu()) return;
    const v = Math.max(-9999, Math.min(9999, Math.round(hz || 0)));
    const sign = v < 0 ? '-' : '+';
    this._write(`CF${sign}${String(Math.abs(v)).padStart(4, '0')};`);
  }
