...
class RemoteServer extends EventEmitter {
  ...
  _handleMessage(ws, msg) {
    ...
    switch (msg.type) {
      ...
      case 'tx-eq-get':
        ...
        break;
      case 'tx-eq-set':
        this.emit('tx-eq-set', { enabled: !!msg.enabled, preset: msg.preset || 'ragchew' });
        break;
  ...
  broadcastTxEqState(payload) {
    if (!payload) return;
    if (this._client && this._client.readyState === WebSocket.OPEN) {
      this._sendTo(this._client, { type: 'tx-eq-state', enabled: !!payload.enabled, preset: payload.preset || 'ragchew' });
    }
  }
