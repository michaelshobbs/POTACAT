...
class SstvDecoder {
  ...
  _emitImage() {
    ...
    if (!this._decodeLooksReal()) {
      ...
      const syncPct = Math.round((this._linesWithSync / total) * 100);
      this.reset();
      return {
        type: 'rx-debug',
        state: 'IDLE',
        avgFreq: 0,
        detail: 'Decode discarded as noise (sync on ' + syncPct + '% of lines)',
      };
    }
    const result = {
      type: 'rx-image',
  ...
  // column. Noise produces sparse, randomly-scattered sync peaks.
  _decodeLooksReal() {
    const total = this.mode ? this.mode.height : 0;
    if (total <= 0) return false;
    if (this._linesWithSync / total < 0.5) return false;
    ...
    const spread = Math.sqrt(varSum / cols.length) / lineWidth;
    // Real sync sits at a near-constant column (gradual slant only); noise
    // scatters uniformly across the line, std ~= 0.29 of the width. 0.15
