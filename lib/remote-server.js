...
class RemoteServer extends EventEmitter {
  constructor() {
    ...
    // After the client tunes, suppress freq in outgoing status messages
    // briefly so the next CAT poll (which returns the rig's freq from ~one
    // wheel-tick ago) can't snap the client's optimistic display backwards
    // mid-spin. W8IJW (IC-7300, web ECHOCAT) and K6RBJ (phone + desktop)
    // 2026-05-24. We replace freq with the client's own target so the rest
    // of the status payload (mode, smeter, etc.) still flows live.
    this._postTuneFreqSuppressUntil = 0;
    this._postTuneFreqHz = 0;
  ...
  _handleMessage(ws, msg) {
    ...
    switch (msg.type) {
      case 'tune': {
        ...
        // Suppress freq snap-back: arm the window so subsequent status
        // broadcasts echo the client's tune target back instead of whatever
        // the next CAT poll happens to read mid-settling.
        this._postTuneFreqSuppressUntil = now + 1200;
        this._postTuneFreqHz = Math.round(freqKhz * 1000);
        this.emit('tune', {
          freqKhz,
          mode: msg.mode,
          bearing: msg.bearing,
  ...
  broadcastRadioStatus(status) {
    ...
    if (this._client && this._client.readyState === WebSocket.OPEN) {
      ...
      // During the post-tune freq-suppress window, send the client's tune
      // target instead of the cache's polled freq. The rest of the snapshot
      // (mode, smeter, swr, etc.) still flows live.
      if (Date.now() < this._postTuneFreqSuppressUntil && this._postTuneFreqHz > 0) {
        out.freq = this._postTuneFreqHz;
      }
