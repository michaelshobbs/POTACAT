...
const MESSAGES = Object.freeze({
  ...
  // The mobile app's Settings → Diagnostics assembles a shared bug report
  // from BOTH ends. The phone sends `request-diagnostic` and awaits a
  // `diagnostic-snapshot` carrying the SAME requestId; the DESKTOP is the
  // responder (this side). `redact:true` means the snapshot is safe to
  // paste into a public report (callsign/tokens/hosts/paths masked).
  //
  // `sections` is an any-bag object: its shape mirrors the mobile
  // BugReportAssembler HostSnapshot and evolves independently, so it is
  // intentionally untyped here. Every field except requestId is optional
  // so a partial/error snapshot still validates (a refused or failed
  // gather replies with `error` and no sections rather than going silent,
  // which would strand the phone on its 5s timeout).
  'request-diagnostic': { dir: Dir.C2S, feature: 'core', fields: { requestId: f.string, redact: f.optBoolean } },
  'diagnostic-snapshot': { dir: Dir.S2C, feature: 'core', fields: { requestId: f.string, source: f.optString, appVersion: f.optString, platform: f.optString, timestamp: f.optNumber, sections: f.optObject, error: f.optString } },
