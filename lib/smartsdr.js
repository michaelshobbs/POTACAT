...
class SmartSdrClient extends EventEmitter {
  ...
  subscribeSliceAudio(sliceIdx) {
    ...
    // Modern Flex API: stream create returns a stream handle that
    // subsequent audio packets carry as their stream ID.
    this._send('stream create type=remote_audio_rx');
    // Bind the just-created stream to the requested slice. Both
    // commands together produce slice audio routed to our UDP port.
    setTimeout(() => {
      if (!this.connected) return;
      this._send(`audio client 0 slice ${sliceIdx} 1`);
    }, 200);
  ...
  unsubscribeSliceAudio() {
    if (this._audioStreamIds) {
      for (const id of this._audioStreamIds) {
        try { this._send(`stream remove ${id.toString(16)}`); } catch {}
      }
  ...
  _maybeCaptureAudioStreamId(line) {
    ...
    // Watchdog: if no audio packets arrive within 5s, emit a fallback
    // signal so main.js can drop back to DAX.
