...
class SstvDecoder {
  ...
  // Linear-interp resample of the raw audio buffer to undo encoder clock
  // drift. ratio > 1 means we INSERT samples (input was sped up by drifted
  // encoder); ratio < 1 means we DROP samples.
  //
  // Sinc (Lanczos-3, 16-tap) was tried (2026-05-31) — gained 1-1.5 dB
  // on Martin/Robot36 but cost 6× CPU and regressed Scottie1 by 0.5 dB
  // (ringing artifacts on Scottie's sharp G→B channel transitions).
  // Linear with ~12s full-matrix runtime is the right tradeoff for
  // real-world live decoding.
