...
class SmartSdrClient extends EventEmitter {
  ...
  _handleLine(line) {
    // Capture audio stream IDs from `stream create` replies before
    // any other parsing — replies can land on R-prefixed lines that
    // would otherwise short-circuit out of the parser early.
    if (line.includes('stream_id=')) this._maybeCaptureAudioStreamId(line);
    ...
    if (rMatch) {
      ...
      if (status !== 0 && status !== 0x50001000) {
        ...
        // Audio subscribe rejected by Flex (most often 0x500000aa —
        // wrong client scope, or older firmware without the command).
        // Fire fallback immediately rather than waiting for the 5 s
        // no-frames watchdog: the user shouldn't sit silent.
        if (this._audioCmdSeqs && this._audioCmdSeqs.has(seq)) {
          this._audioCmdSeqs.delete(seq);
          if (!this._audioFrameSeen) {
            if (this._audioFrameTimer) {
              clearTimeout(this._audioFrameTimer);
              this._audioFrameTimer = null;
            }
            this.emit('log', `Audio subscribe rejected (status 0x${status.toString(16)}) — emitting audio-fallback`);
            this.emit('audio-fallback');
          }
        }
      }
    }
  ...
  // Single dispatcher for VITA-49 packets arriving on our UDP port.
  // Flex sends meters AND any subscribed audio streams to the same
  // port we registered with `client udpport`. Dispatch by stream ID.
  _parseUdpPacket(buf) {
    ...
    } else if (this._audioStreamIds && this._audioStreamIds.has(streamId)) {
      this._parseAudioPacket(buf, streamId);
    }
  ...
  // --- Slice audio subscription (DAX-free path) -------------------------
  //
  // Flex emits non-DAX slice audio as VITA-49 UDP packets to whatever
  // client UDP port we registered with `client udpport`. Subscribing
  // bypasses Windows DAX entirely — particularly useful for users
  // operating over RDP, where DAX can get into "connected but silent"
  // states that POTACAT can't recover from.
  //
  // Architecture: we send the Flex a `stream create` command, capture
  // the resulting stream ID from its reply, then route incoming
  // VITA-49 packets with that stream ID through _parseAudioPacket,
  // which emits raw Opus frames as 'audio-frame' events. Decoding
  // happens in the renderer (WebCodecs) — keeps this module Node-only,
  // no native deps.
  subscribeSliceAudio(sliceIdx) {
    if (!this.connected) {
      this.emit('log', 'Audio subscribe deferred — not connected yet');
      return;
    }
    if (!this._audioStreamIds) this._audioStreamIds = new Set();
    if (!this._audioCmdSeqs) this._audioCmdSeqs = new Set();
    this._audioPendingSlice = sliceIdx;
    this._audioFrameSeen = false;

    // Older Flex API form, predates the GUI/non-GUI client model.
    // Newer `audio client 0 slice N 1` requires a non-GUI scope, but
    // the existing TCP connection is bound to a GUI client (for CW),
    // so the Flex rejects the modern command with 0x500000aa
    // ("Invalid command for this client type"). The legacy `audio
    // stream <slice> dax=0` form is accepted in either scope.
    const seq1 = this._send('stream create type=remote_audio_rx');
    if (seq1 != null) this._audioCmdSeqs.add(seq1);
    setTimeout(() => {
      if (!this.connected) return;
      const seq2 = this._send(`audio stream ${sliceIdx} dax=0`);
      if (seq2 != null) this._audioCmdSeqs.add(seq2);
    }, 200);
    this.emit('log', `Audio subscribe requested for slice ${sliceIdx}`);

    // Arm the watchdog at subscribe time, not at stream_id capture.
    // Two reasons:
    //   1. If the Flex rejects the subscribe entirely (older firmware,
    //      wrong scope), no stream_id is ever returned and the old
    //      capture-time watchdog never armed → user stuck silent.
    //   2. The cmd-error path now triggers fallback immediately on
    //      subscribe rejection, so this watchdog only matters for
    //      "subscribed successfully but no frames" — but that's still
    //      the normal failure mode for a misconfigured slice.
    if (this._audioFrameTimer) clearTimeout(this._audioFrameTimer);
    this._audioFrameTimer = setTimeout(() => {
      if (!this._audioFrameSeen) {
        this.emit('log', 'No SmartSDR audio frames in 5s — emitting audio-fallback');
        this.emit('audio-fallback');
      }
    }, 5000);
  }

  unsubscribeSliceAudio() {
    if (this._audioStreamIds) {
      for (const id of this._audioStreamIds) {
        try { this._send(`stream remove 0x${id.toString(16)}`); } catch {}
      }
      this._audioStreamIds.clear();
    }
    if (this._audioCmdSeqs) this._audioCmdSeqs.clear();
    this._audioPendingSlice = null;
    this._audioFrameSeen = false;
    if (this._audioFrameTimer) {
      clearTimeout(this._audioFrameTimer);
      this._audioFrameTimer = null;
  ...
  // Capture a stream-create reply so we know which incoming UDP
  // packets carry our audio. Flex replies look like:
  //   R0|0|stream_id=0xXXXXXXXX
  // Called from the TCP-line parser on any line containing stream_id.
  _maybeCaptureAudioStreamId(line) {
    if (this._audioPendingSlice == null) return;
    const m = line.match(/stream_id=0x([0-9a-fA-F]+)/);
    if (!m) return;
    const streamId = parseInt(m[1], 16);
    if (!this._audioStreamIds) this._audioStreamIds = new Set();
    this._audioStreamIds.add(streamId);
    this._audioPendingSlice = null;
    this.emit('log', `Audio stream id 0x${streamId.toString(16)} bound for slice audio`);
    // Watchdog is armed at subscribe time — see subscribeSliceAudio.
    // We get here only on a successful stream create; if the audio
    // stream command then succeeds and frames arrive, _parseAudioPacket
    // clears the timer. If the audio stream command is rejected, the
    // cmd-error handler fires fallback immediately. If the stream
    // create succeeded but no frames flow within 5 s (subscribed but
    // silent slice), the subscribe-time watchdog catches it.
  }

  _parseAudioPacket(buf, streamId) {
    // VITA-49 header: 7×4 bytes = 28 bytes. Payload follows.
    // Audio packets are Opus-encoded; the payload IS the Opus frame
    // (no further wrapping) — pass through to renderer for decode.
    if (buf.length <= 28) return;
    const payload = buf.slice(28);
    if (!this._audioFrameSeen) {
      this._audioFrameSeen = true;
      if (this._audioFrameTimer) {
        clearTimeout(this._audioFrameTimer);
        this._audioFrameTimer = null;
      }
