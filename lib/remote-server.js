...
function getOrCreateTlsCert(certDir, opts = {}) {
  // Caller-provided cert path takes priority. This is the path for
  // users who run `tailscale cert <hostname>` (or any other
  // publicly-trusted cert) and point POTACAT at the resulting files.
  // iOS trusts the Tailscale-issued LE cert natively, sidestepping
  // the entire self-signed / ATS rejection problem. K3SBP 2026-05-05.
  if (opts.userCertPath && opts.userKeyPath) {
    try {
      ...
      console.warn(`[Echo CAT] Failed to read user-provided TLS cert (${err.message}) — falling back to self-signed.`);
    }
  }
