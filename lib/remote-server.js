...
class RemoteServer extends EventEmitter {
  ...
  _sendWorkedQsosCapped(ws) {
    ...
    if (json.length > MAX_BYTES) {
      ...
      console.log(`[Echo CAT] Skipping worked-qsos auto-push — ${json.length} bytes / ${callCount} calls exceeds ${MAX_BYTES}-byte cap. Phone will not see per-call QSO history.`);
      this._sendTo(ws, { type: 'worked-qsos-skipped', reason: 'size', bytes: json.length, cap: MAX_BYTES });
      return;
    }
    try { ws.send(json); } catch {} // already serialized — reuse it
  }
