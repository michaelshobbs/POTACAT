  ...
  _dialNext(candidates, idx) {
    ...
    ws.on('open', () => {
      ...
      this._send(protocol.buildClientHello({
        ...
        // 'diagnostic-snapshot': this desktop (in remote-client mode)
        // can answer a request-diagnostic from its host for the Unified
        // Bug Report, so the host short-circuits instead of timing out.
        capabilities: ['qso-attributed', 'log-error', 'diagnostic-snapshot'],
      }));
    });
